...
static int sanitizeNumAmostrasMedia(int value) {
  return constrain(value, 1, 20);
}

static bool isValidConversionFactor(float value) {
  return isfinite(value) && fabsf(value) >= 1e-6f;
}
    ...
    case CMD_SET_PARAM: {
      ...
      switch (cmd->param_id) {
        ...
        case PARAM_CONV_FACTOR:
          if (isValidConversionFactor(cmd->value_f)) {
        ...
        case PARAM_NUM_AMOSTRAS:
          config.numAmostrasMedia = sanitizeNumAmostrasMedia((int)cmd->value_i);
          updated = true;
...
void loop() {
  ...
  if (millis() - lastReadTime >= 10) {
    ...
    if (loadcell.is_ready()) {
      ...
      if (isValidConversionFactor(config.conversionFactor)) {
  ...
  if (strcmp(cmd, "t") == 0) {
    ...
    if (paramName && commandDoc.containsKey("value")) {
      if (strcmp(paramName, "conversionFactor") == 0) {
        if (isValidConversionFactor(paramValueF)) {
        ...
        config.numAmostrasMedia = sanitizeNumAmostrasMedia(paramValueL);
        changed = true;
      }
...
void loadConfig() {
  ...
  if (tempConfig.magic_number == 123456789) {
    ...
    int sanitizedNumAmostrasMedia = sanitizeNumAmostrasMedia(config.numAmostrasMedia);
    if (config.numAmostrasMedia != sanitizedNumAmostrasMedia) {
