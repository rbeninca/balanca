...
export class TelaJogoBarra {
  private cfgCalib = { desvioPadraoMaxN: 2, duracaoEstavelS: 3 };
  private calibrador  = new CalibradorBarra(100, this.cfgCalib);
  ...
  private elCalibForca:  HTMLElement | null = null;
  private elCalibPeso:   HTMLElement | null = null;

  // Refs controles
  private elBtnReset:   HTMLButtonElement | null = null;
  private elTolerancia: HTMLInputElement  | null = null;
  private elDuracao:    HTMLInputElement  | null = null;
  ...
  private renderizar(container: HTMLElement): void {
    ...
    container.innerHTML = `
      ...
      <div class="barra-wrapper">
        <div class="barra-canvas-container">
          ...
          <div class="barra-hud">
            <div class="barra-hud-calib hidden" id="barra-calib">
              ...
              <div class="barra-calib-valores">
                <span class="barra-calib-vlabel">Força</span>
                <span class="barra-calib-vval" id="barra-calib-forca">— N</span>
                <span class="barra-calib-sep">|</span>
                <span class="barra-calib-vlabel">Peso estimado</span>
                <span class="barra-calib-vval" id="barra-calib-peso">— kg</span>
              </div>
        ...
        <div class="barra-controles">
          <label class="barra-cfg-item">
            <span class="barra-cfg-nome">Tolerância</span>
            <input class="barra-cfg-input" type="number" id="barra-tolerancia"
              value="${this.cfgCalib.desvioPadraoMaxN}" min="1" max="50" step="1">
            <span class="barra-cfg-unidade">N</span>
          </label>
          <label class="barra-cfg-item">
            <span class="barra-cfg-nome">Duração</span>
            <input class="barra-cfg-input" type="number" id="barra-duracao"
              value="${this.cfgCalib.duracaoEstavelS}" min="1" max="15" step="1">
            <span class="barra-cfg-unidade">s</span>
          </label>
    ...
    this.elCalibForca = container.querySelector('#barra-calib-forca');
    this.elCalibPeso  = container.querySelector('#barra-calib-peso');
    this.elBtnReset   = container.querySelector('#barra-reset');
    this.elTolerancia = container.querySelector('#barra-tolerancia');
    this.elDuracao    = container.querySelector('#barra-duracao');

    this.elBtnReset?.addEventListener('click', () => this.resetar());
    this.elTolerancia?.addEventListener('change', () => this.onCfgChange());
    this.elDuracao?.addEventListener('change',    () => this.onCfgChange());
  ...
  private onCfgChange(): void {
    const tol = Number(this.elTolerancia?.value ?? this.cfgCalib.desvioPadraoMaxN);
    const dur  = Number(this.elDuracao?.value    ?? this.cfgCalib.duracaoEstavelS);
    this.cfgCalib = {
      desvioPadraoMaxN: Math.max(1, tol),
      duracaoEstavelS:  Math.max(1, dur),
    };
    this.calibrador = new CalibradorBarra(100, this.cfgCalib);
    this.resetar(false);
  }
  ...
  private desenhar(): void {
    ...
    const ESCALA_M      = (H - MARGEM_TOP - MARGEM_BOTTOM) / 2;
  ...
  private atualizarHUD(): void {
    ...
    if (!this.calibrador.calibrado) {
      ...
      if (this.elCalibForca) this.elCalibForca.textContent    = `${this.forcaAtualN.toFixed(1)} N`;
      if (this.elCalibPeso && this.mediaCalibN > 0) {
        this.elCalibPeso.textContent = `${(this.mediaCalibN / 9.81).toFixed(1)} kg`;
      }
      return;
    }
  ...
  private resetar(mostrarCalib = true): void {
